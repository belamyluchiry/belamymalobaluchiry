 //BelamyLuchiryFEE3/2552/2024 project no.56 collaborativefilteringrecommender
#include <iostream>
#include <vector>
#include <iomanip>
#include <map>
...
if(den == 0){ return 0.0;}else return num/den;
    ...
    void explainRecommendation(const string& userName, const string& itemName) {
        ...
        for (auto& r : users[userId]->ratings) {
            ...
            if (sim > 0) {
                found = true;
                cout << "-It's Similar to " << items[r.first]->name<< " (the user rated " << r.second
    ...
    void Menu() {
        ...
        do {
            ...
            cout <<"KINDLY TAKE NOTE that lest more items are added by user the available items(movies) in the system are:\n";
            cout << "Oppenheimer,Ted,Moneyheist,Inspector,Friends and Familyguy,\n";
            cout << "The users in the system are Belamy,Peter,Luchiry and patrick feel free to add more though"<<endl;
    ...
    void recommendColdStartUser(const string& userName, int topN = 5) {
        if (userNameToIdentifier.find(userName) == userNameToIdentifier.end()) {
            cout << "User not found...Please add new user first (7);\n";
            return;
        }
...
<< avgRatings[i].second->name << " (Average Rating: " << avgRatings[i].first << ")\n";
 ...
 void suggestUsersForNewItem(const string& itemName, int topN = 5) {
        if (itemNameToIdentifier.find(itemName) == itemNameToIdentifier.end()) {
                         cout <<"Item is not found...Please add new item first\n";
            return;}
...
system.addRating("Belamy", "Oppenheimer", 4.5);
     ...
     system.addRating("Belamy", "Moneyheist", 3.5);  ;
           system.addRating("Peter", "Inspector", 4.0);
...
system.addRating("Luchiry", "Oppenheimer", 5.0);
 ...
 system.addRating("Luchiry", "Inspector", 4.5);
          system.Menu();
                   return 0;
}